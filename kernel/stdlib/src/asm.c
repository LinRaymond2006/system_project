...
volatile extern inline sti() {
	...
	printf("information: interrupt enabled. remember to lock shared resources or race condition by interrupt handler will happen!\n");
    return;
}
volatile extern inline cli() {
    __asm__ __volatile__ ("cli":::"memory");
	printf("information: interrupt disabled\n");
    return;
}